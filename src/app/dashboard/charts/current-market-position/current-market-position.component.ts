...
export class CurrentMarketPositionComponent implements OnInit {
  ...
  chartOptions = {
    ...
    data: [{
      ...
      yValueFormatString: "#,###.##'%'",
      dataPoints: [
        { y: 0, name: "FIIs", total: 0 },
        { y: 0, name: "Stocks", total: 0 }
      ]
    }]
  };
  ...
  getMonthlyTransactions(year: any) {
    this.transactionsService.list(year)
      .subscribe((rsp: any = {}) => {
        ...
        this.chartOptions.data[0].dataPoints[0].total = Number(totalFIIs);
        this.chartOptions.data[0].dataPoints[1].total = Number(totalStocks);
